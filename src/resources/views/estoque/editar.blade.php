...
@section('conteudo')
    <div class="bg-white p-4 rounded-md w-full max-w-4xl mx-auto">
        ...
        <form action="{{ route('estoque.salvarEditar', $estoque->id_estoque) }}" method="POST" class="space-y-6"
            id="form-estoque-editar">
            @csrf
            @method('PUT')

            {{-- hidden p/ impostos (preenchidos após o preview via AJAX) --}}
            ...
            <div class="grid md:grid-cols-2 gap-6">
                <div class="form-group">
                    ...
                    <select name="id_fornecedor_fk"
                        class="w-full px-3 py-2 border rounded-md focus:ring-2 focus:ring-cyan-500">
                        @foreach ($fornecedores as $fornecedor)
                            <option value="{{ $fornecedor->id_fornecedor }}" {{ (int) old('id_fornecedor_fk', $estoque->id_fornecedor_fk) === (int) $fornecedor->id_fornecedor ? 'selected' : '' }}>
                ...
                <div class="form-group">
                    ...
                    <select name="id_marca_fk" class="w-full px-3 py-2 border rounded-md focus:ring-2 focus:ring-cyan-500">
                        @foreach ($marcas as $marca)
                            <option value="{{ $marca->id_marca }}" {{ (int) old('id_marca_fk', $estoque->id_marca_fk) === (int) $marca->id_marca ? 'selected' : '' }}>
                                {{ $marca->nome_marca }}
                            </option>
                        @endforeach
                    </select>
