  ...
  <div class="bg-white p-4 rounded-md w-full">
    ...
    <form action="{{ route('fornecedor.buscar') }}" method="GET" class="mb-6">
      <div class="flex gap-2 w-full md:w-1/2">
        ...
        <button type="submit" class="px-6 py-2 bg-cyan-500 hover:bg-cyan-600 text-white rounded-full transition-colors">
          Pesquisar
        </button>
      </div>
    </form>
          ...
          @foreach ($fornecedores as $fornecedor)
            <tr class="hover:bg-gray-50">
              ...
              <td class="px-4 py-3 text-sm flex gap-2">
                <x-edit-button :route="'fornecedor.editar'" :modelId="$fornecedor->id_fornecedor" menuSlug="fornecedores" />
                <x-button-status :modelId="$fornecedor->id_fornecedor" :status="$fornecedor->status"
