    ...
    <div class="flex justify-center p-4 bg-gray-100 min-h-screen">
        <div class="bg-white rounded-lg shadow-lg p-4 grid grid-cols-1 md:grid-cols-3 gap-4 max-w-screen-xl w-full">
            <div class="border rounded-lg p-4">
                ...
                <div v-if="session.connected"
                    class="mt-4 p-3 rounded border bg-green-50 text-sm flex items-center justify-between">
                    <div>
                        <div class="font-semibold text-green-700">Conectado</div>
                        <div class="text-gray-700">
                            {{ session.pushname ? session.pushname + ' — ' : '' }}
                            {{ session.phone ? '+' + session.phone : 'Número indisponível' }}
                        </div>
                    </div>
                    <button class="bg-red-600 text-white px-3 py-1 rounded" @click="logoutWhatsApp">
                        Desconectar
                    </button>
                </div>

                <!-- Se não conectado, mostra o botão para conectar -->
                <button v-else class="bg-green-600 text-white px-4 py-2 rounded mt-4" @click="showQrModal = true">
                    Conectar WhatsApp
                </button>
            ...
            <div class="border rounded-lg p-4">
                ...
                <div class="relative group">
                    <select v-model="selectedTemplateId" @change="applyTemplate"
                        ...
                        <option v-for="tpl in templates" :key="tpl.id" :value="tpl.id" @mouseenter="showPreview(tpl)"
            ...
            <div class="border rounded-lg p-4">
                ...
                <input v-model="scheduledTime" type="datetime-local" class="border rounded p-1 w-full text-sm mb-2" />
...
export default {
    ...
    data() {
        return {
            ...
            pollId: null,
            sourceType: 'whatsapp',
            session: { connected: false, phone: null, pushname: null },
        }
    },
    mounted() {
        this.refreshSession();
    ...
    methods: {
        ...
        async refreshSession() {
            try {
                const r = await fetch(`${BASE_URL}/verdurao/bot/whatsapp/session`);
                const j = await r.json();
                this.session = j || { connected: false };
                this.isConnected = !!this.session.connected; // reaproveita seu flag atual
            } catch {
        ...
        async logoutWhatsApp() {
            try {
                await fetch(`${BASE_URL}/verdurao/bot/whatsapp/logout`, { method: 'POST' });
                this.session = { connected: false, phone: null, pushname: null };
                this.isConnected = false;
                this.qrcode = null;
                this.showQrModal = false;
                this.feedback = 'Sessão desconectada.';
                this.feedbackType = 'success';
            } catch (e) {
                this.feedback = 'Erro ao desconectar.';
                this.feedbackType = 'error';
            }
        },
        async getQrCode() {
            await this.refreshSession();
            if (this.session.connected) { this.isConnected = true; this.qrcode = null; return; }
