...
<template>
  <form @submit.prevent="emit('submit')" class="grid grid-cols-1 md:grid-cols-2 gap-4 bg-white p-4 rounded shadow">
    ...
    <div>
      <label class="block text-sm font-medium">{{ $t('Tipo de Pessoa') }}</label>
    ...
    <div>
      <label class="block text-sm font-medium">{{ $t('Documento (CPF/CNPJ)') }}</label>
    ...
    <div v-if="form.tipo_pessoa==='PJ'">
      <label class="block text-sm font-medium">{{ $t('Nome Fantasia') }}</label>
    ...
    <div>
      <label class="block text-sm font-medium">{{ $t('E-mail') }}</label>
      <input v-model="form.email" type="email" class="mt-1 border rounded px-3 py-2 w-full" />
    </div>

    <div>
      <label class="block text-sm font-medium">{{ $t('WhatsApp') }}</label>
    ...
    <div class="md:col-span-2 pt-2 border-t">
      ...
      <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div><label class="block text-sm">{{ $t('CEP') }}</label><input v-model="form.cep" class="mt-1 border rounded px-3 py-2 w-full" /></div>
        <div class="md:col-span-2"><label class="block text-sm">{{ $t('Logradouro') }}</label><input v-model="form.logradouro" class="mt-1 border rounded px-3 py-2 w-full" /></div>
        <div><label class="block text-sm">{{ $t('Número') }}</label><input v-model="form.numero" class="mt-1 border rounded px-3 py-2 w-full" /></div>
        <div><label class="block text-sm">{{ $t('Complemento') }}</label><input v-model="form.complemento" class="mt-1 border rounded px-3 py-2 w-full" /></div>
        <div><label class="block text-sm">{{ $t('Bairro') }}</label><input v-model="form.bairro" class="mt-1 border rounded px-3 py-2 w-full" /></div>
        <div><label class="block text-sm">{{ $t('Cidade') }}</label><input v-model="form.cidade" class="mt-1 border rounded px-3 py-2 w-full" /></div>
        <div>
          <label class="block text-sm">{{ $t('UF') }}</label>
    ...
    <div>
      <label class="block text-sm font-medium">{{ $t('Segmento') }}</label>
    ...
    <div>
      <label class="block text-sm font-medium">{{ $t('Tabela de Preço') }}</label>
    ...
    <div>
      <label class="block text-sm font-medium">{{ $t('Imposto Padrão') }}</label>
    ...
    <div>
      <label class="block text-sm font-medium">{{ $t('Limite de Crédito') }}</label>
    ...
    <div class="flex items-center gap-2">
      ...
      <label for="bloq">{{ $t('Bloqueado') }}</label>
    </div>
    ...
    <div class="md:col-span-2">
      <label class="block text-sm font-medium">{{ $t('Endereço de Faturação') }}</label>
    ...
    <div class="md:col-span-2">
      <label class="block text-sm font-medium">{{ $t('Endereço de Entrega') }}</label>
    ...
    <div class="md:col-span-2">
      <label class="block text-sm font-medium">{{ $t('Observações') }}</label>
      <textarea v-model="form.observacoes" rows="3" class="mt-1 border rounded px-3 py-2 w-full" />
    </div>

    <div class="md:col-span-2 flex justify-end gap-2">
      <button class="px-3 py-2 rounded bg-blue-600 text-white" :disabled="form.processing">{{ $t('Save') }}</button>
    </div>
  </form>
</template>
