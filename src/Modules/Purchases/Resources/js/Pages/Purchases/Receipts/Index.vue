...
const dtColumns = computed(() => [
  ...
  { data: 'c3', title: t('Order') },
  { data: 'c4', title: t('Supplier') },
  { data: 'c5', title: t('Date') },
...
<template>
  ...
  <div class="flex justify-between items-center mb-6">
    <h2 class="text-2xl font-semibold text-slate-700">{{ $t('Receipts') }}</h2>
  ...
  <div class="mb-6 mt-3 grid grid-cols-1 md:grid-cols-5 gap-2">
    ...
    <select v-model="form.status" class="px-3 py-2 border rounded-md focus:outline-none focus:ring-2 focus:ring-cyan-500">
      ...
      <option value="registrado">{{ $t('Registered') }}</option>
      <option value="conferido">{{ $t('Checked') }}</option>
      <option value="com_divergencia">{{ $t('With Divergence') }}</option>
  ...
  <DataTable
    table-id="dt-purchases-receipts"
    :ajax-url="route('purchases.receipts.data')"
    :ajax-params="form"
    :columns="dtColumns"
    :order="[[0, 'desc']]"
    :page-length="10"
