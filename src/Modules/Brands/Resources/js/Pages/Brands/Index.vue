...
const stopSyncFilters = useQueryFilters(form, 'marca.index')
onBeforeUnmount(() => stopSyncFilters())

const { t } = useI18n()

const dtColumns = computed(() => [
  { data: 'c1', title: t('Brand') },
  {
    data: 'st',
    title: t('Status'),
    render: (data) => data
...
<template>
  ...
  <div class="flex justify-between items-center mb-6">
    <h2 class="text-2xl font-semibold text-slate-700">{{ $t('Brands') }}</h2>
    <div class="flex gap-4">
      ...
      <Link :href="route('marca.cadastro')" class="flex items-center px-4 py-2 bg-gray-100 hover:bg-gray-200 rounded-md text-gray-800 transition-colors">
