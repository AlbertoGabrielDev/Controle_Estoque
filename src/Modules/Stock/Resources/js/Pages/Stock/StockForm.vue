...
<template>
  <form @submit.prevent="$emit('submit')" class="bg-white p-4 rounded shadow space-y-4">
    <div v-if="props.lockProduct">
      <label class="block text-sm font-medium">{{ $t('Product') }}</label>
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
      <div>
        <label class="block text-sm font-medium">{{ $t('Qty') }}</label>
      ...
      <div>
        <label class="block text-sm font-medium">{{ $t('Sale') }}</label>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
      <div>
        <label class="block text-sm font-medium">{{ $t('Lot QRCode') }}</label>
        <input
          v-model="props.form.qrcode"
          class="mt-1 border rounded px-3 py-2 w-full bg-gray-100"
          :placeholder="$t('Will be generated upon saving')"
          readonly
        >
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
      <div>
        <label class="block text-sm font-medium">{{ $t('Warning Quantity') }}</label>
      ...
      <div>
        <label class="block text-sm font-medium">{{ $t('Lot') }}</label>
      ...
      <div>
        <label class="block text-sm font-medium">{{ $t('Location') }}</label>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
      <div>
        <label class="block text-sm font-medium">{{ $t('Expiration Date') }}</label>
      ...
      <div>
        <label class="block text-sm font-medium">{{ $t('Arrival Date') }}</label>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
      ...
      <div>
        <label class="block text-sm font-medium">{{ $t('Brand') }}</label>
    ...
    <div class="mt-6 border rounded p-4">
      <div class="flex items-center justify-between">
        <h3 class="text-lg font-semibold text-slate-700">{{ $t('Estimated Taxes') }}</h3>
      </div>
    ...
    <div class="flex justify-end gap-2">
      <button :disabled="props.form.processing" class="px-3 py-2 rounded bg-blue-600 text-white">{{ props.submitLabel || $t('Save') }}</button>
    </div>
  </form>
</template>
