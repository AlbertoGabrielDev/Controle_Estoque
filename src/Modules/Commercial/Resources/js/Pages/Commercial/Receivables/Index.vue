...
const dtColumns = computed(() => [
  ...
  { data: 'c1', title: t('Document') }, { data: 'c2', title: t('Status') }, { data: 'c3', title: t('Customer') },
  { data: 'c4', title: t('Issue Date') }, { data: 'c5', title: t('Due Date') }, { data: 'c6', title: t('Value') },
  { data: 'acoes', title: t('Actions'), orderable: false, searchable: false },
])

const stopSync = useQueryFilters(form, 'commercial.receivables.index')
onBeforeUnmount(() => stopSync())
</script>

<template>
  <Head :title="$t('Accounts Receivable')" />
  <div class="flex justify-between items-center mb-6">
    <h2 class="text-2xl font-semibold text-slate-700 dark:text-slate-100">{{ $t('Accounts Receivable') }}</h2>
  </div>
  ...
  <DataTable table-id="dt-commercial-receivables" :ajax-url="route('commercial.receivables.data')" :ajax-params="form" :columns="dtColumns" :order="[[0, 'desc']]" :page-length="15" :actions-col-index="7" />
</template>
