...
function onItemSelected(val, index) {
  ...
  if (val) {
    row.item_id = val.id
    row.descricao_snapshot = val.nome
    row.preco_unit = val.preco_venda ?? 0
    if (val.unidade_medida_id) {
      row.unidade_medida_id = val.unidade_medida_id
      row._unidade_obj = props.unidadesOptions.find(u => u.id === val.unidade_medida_id) ?? null
    }
  } else {
    row.item_id = ''
  }
...
function onUnidadeSelected(val, index) {
  props.form.items[index].unidade_medida_id = val?.id ?? ''
}

function onImpostoSelected(val, index) {
  const row = props.form.items[index]
  row.imposto_id = val?.id ?? ''
  row.aliquota_snapshot = val?.aliquota ?? 0
  recalcLine(index)
}
...
function recalcLine(index) {
  ...
  row.total_linha = Math.round((subtotal + subtotal * aliquota / 100) * 100) / 100
}

function onDescontoPercent(index) {
  const row = props.form.items[index]
  const base = Number(row.preco_unit) * Number(row.quantidade)
  row.desconto_valor = Math.round(base * (Number(row.desconto_percent) / 100) * 100) / 100
  recalcLine(index)
}

const subtotal = computed(() =>
  props.form.items.reduce((s, i) => s + Number(i.preco_unit) * Number(i.quantidade), 0)
)
const descontoTotal = computed(() =>
  props.form.items.reduce((s, i) => s + Number(i.desconto_valor), 0)
)
const totalImpostos = computed(() =>
  props.form.items.reduce((s, i) => {
    const base = Number(i.preco_unit) * Number(i.quantidade) - Number(i.desconto_valor)
    return s + base * Number(i.aliquota_snapshot) / 100
  }, 0)
)
...
<template>
  <form @submit.prevent="emit('submit')" class="bg-white p-4 rounded shadow dark:bg-slate-900 dark:border dark:border-slate-700 space-y-4 text-slate-700 dark:text-slate-100">
    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
      ...
      <div>
        <label class="block text-sm font-medium">{{ $t('Validity') }}</label>
        <input v-model="form.validade_ate" type="date" class="mt-1 border rounded px-3 py-2 w-full dark:bg-slate-800 dark:border-slate-700 dark:text-slate-100">
      </div>
    ...
    <div class="flex items-center justify-between">
      <h3 class="font-semibold">{{ $t('Proposal Items') }}</h3>
      ...
      <table class="w-full text-sm border dark:border-slate-700 text-slate-700 dark:text-slate-200">
        ...
        <tbody>
          <tr v-for="(item, index) in form.items" :key="index" class="border-t dark:border-slate-700">
            ...
            <td class="px-3 py-2">
              <input v-model="item.descricao_snapshot" class="border rounded px-2 py-1 w-full text-xs dark:bg-slate-800 dark:border-slate-700 dark:text-slate-100">
            </td>
            <td class="px-3 py-2">
            ...
            <td class="px-3 py-2">
              <input v-model="item.quantidade" type="number" step="0.001" min="0.001" class="border rounded px-2 py-1 w-full dark:bg-slate-800 dark:border-slate-700 dark:text-slate-100" @input="recalcLine(index)">
            </td>
            <td class="px-3 py-2">
              <input v-model="item.preco_unit" type="number" step="0.01" min="0" class="border rounded px-2 py-1 w-full dark:bg-slate-800 dark:border-slate-700 dark:text-slate-100" @input="recalcLine(index)">
            </td>
            <td class="px-3 py-2">
              <input v-model="item.desconto_percent" type="number" step="0.01" min="0" max="100" class="border rounded px-2 py-1 w-full dark:bg-slate-800 dark:border-slate-700 dark:text-slate-100" @input="onDescontoPercent(index)">
            </td>
            <td class="px-3 py-2">
              <input v-model="item.desconto_valor" type="number" step="0.01" min="0" class="border rounded px-2 py-1 w-full dark:bg-slate-800 dark:border-slate-700 dark:text-slate-100" @input="recalcLine(index)">
            </td>
            <td class="px-3 py-2">
            ...
            <td class="px-3 py-2">
              <button type="button" @click="removeItem(index)" class="px-2 py-1 rounded bg-red-50 text-red-600 hover:bg-red-100 dark:bg-red-900/40 dark:text-red-300 text-xs">×</button>
            </td>
          </tr>
          <tr v-if="!form.items.length">
            <td colspan="10" class="px-3 py-3 text-center text-slate-500 dark:text-slate-400">{{ $t('No items added.') }}</td>
          </tr>
        </tbody>
      </table>
    </div>
