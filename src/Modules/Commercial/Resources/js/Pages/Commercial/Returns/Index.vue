...
const dtColumns = computed(() => [
  ...
  { data: 'c4', title: t('Return Date') },
  { data: 'acoes', title: t('Actions'), orderable: false, searchable: false },
])

const stopSync = useQueryFilters(form, 'commercial.returns.index')
...
<template>
  ...
  <div class="flex justify-between items-center mb-6">
    <h2 class="text-2xl font-semibold text-slate-700 dark:text-slate-100">{{ $t('Sales Returns') }}</h2>
    <Link :href="route('commercial.returns.create')" class="px-4 py-2 bg-gray-100 hover:bg-gray-200 dark:bg-slate-800 dark:hover:bg-slate-700 rounded-md text-gray-800 dark:text-slate-100 transition-colors">{{ $t('New Return') }}</Link>
  </div>
  ...
  <DataTable table-id="dt-commercial-returns" :ajax-url="route('commercial.returns.data')" :ajax-params="form" :columns="dtColumns" :order="[[0, 'desc']]" :page-length="15" :actions-col-index="5" />
</template>
